...
New sniff checklist: Sniff class + SniffTest + `Fixtures/*.inc` + registration in `BearSunday/ruleset.xml` + a matching `### BearSunday.<Category>.<Name>` section in `README.md` (Trigger/Rule/example, following the existing sections' format).

## Build, Test, and Development Commands

- `composer install`: install PHP_CodeSniffer, Doctrine/Slevomat standards, and PHPUnit.
